 ...
 */
public class IsDivisorOfBinaryRelation<T> implements BinaryRelation<T> {
    private final BinaryOperation<T> remainderOperation;
    private final Relation<T> isZeroRelation;

    public IsDivisorOfBinaryRelation(BinaryOperation<T> remainderOperation, Relation<T> isZeroRelation) {
    ...
    public boolean test(T a, T b) {
        T remainder = remainderOperation.apply(b, a);
